...
#define immJ() do { \
  uint32_t imm20 = (BITS(i, 31, 31) << 19) | \
                   (BITS(i, 19, 12) << 11) | \
                   (BITS(i, 20, 20) << 10) | \
                   (BITS(i, 30, 21) << 0);   \
  *imm = SEXT(imm20, 20) << 1;               \
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000000 ????? ????? 000 ????? 00100 11", mv     , I, R(rd) = src1);
...
//  INSTPAT("0000000 00000 00001 000 00000 11001 11", ret    , I, s->dnpc = src1 & ~1);
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, if (src1 == src2) s->dnpc = s->pc + imm; else s->dnpc = s->pc + 4);
